...
[OpenLayers](http://openlayers.org/) can display map tiles, vector data and markers loaded from any source, and is made for enabling the use of geographic information of all kinds. It is completely free and Open Source. We have primarily tested integration of OpenLayers through the [GeoAdmin](http://api.geo.admin.ch/) service of the Swiss federal government. It is possible to create maps with custom data loaded via WMTS, WMS, GPX, 3Dtiles, Vectortiles MBT or KML, and embed them into our platform. With www.geo.admin.ch/iframe, geo.admin.ch provides a free and easy way to integrate websites. The description of a location, a site plan for an event or the illustration of a fact can be easily visualized. In addition, map contents can be supplemented and, if required, the application as a whole can be integrated as an iFrame.
